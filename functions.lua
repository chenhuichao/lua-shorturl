...
function M.get_url_details(long_url)
	...
	local result = {}
	local matches,err = ngx.re.match(long_url, '^(http|https|ftp)://([^/]+)((/[^\\\\?#]+)([^#]+)?(.+))?')
	
	if err ~= nil or matches == nil then
		return nil,41
	end
	
	if #matches ~= 2 or  matches[1] == nil or matches[2] == nil then
		return nil,41;
	end
